...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  if (card == village)
  {
    cardEffectVillage(currentPlayer, state, handPos);
  }

  if (card == great_hall)
  {
    cardEffectGreatHall(currentPlayer, state, handPos);
  }

  if (card == outpost)
  {
    cardEffectOutpost(currentPlayer, state, handPos);
  }

  if (card == adventurer)
  {
    cardEffectAdventurer(currentPlayer, state, drawntreasure, cardDrawn, temphand, MAX_HAND);
  }
